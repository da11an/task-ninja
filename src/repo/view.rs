...
impl ViewRepo {
    pub fn get_by_name(conn: &Connection, entity: &str, name: &str) -> Result<Option<ListView>> {
        let mut stmt = conn.prepare(
            "SELECT name, entity, filter_json, sort_json, group_json, created_ts, modified_ts
    ...
    pub fn upsert(
        ...
        conn.execute(
            "INSERT INTO list_views (name, entity, filter_json, sort_json, group_json, created_ts, modified_ts)
            ...
            rusqlite::params![name, entity, filter_json, sort_json, group_json, created_ts, now],
        )
